...
from LSTM.LSTM_track_load import predict, model
from LSTM.LSTM_exist_load import exist_predict
...
def LSTM_predict_distance(tracks, detections, img_size=(1280, 720)):
    """
    ...
    valid_track_bbox = [[\
        ...
        if len(bbox_elem) else [] for bbox_elem in bbox]\
...
def fuse_two_costs(cost_matrix1, cost_matrix2):
    if cost_matrix1.size == 0 or cost_matrix2.size == 0:
        return cost_matrix1 if cost_matrix2.size == 0 else cost_matrix2

    sim_matrix1 = 1 - cost_matrix1
    sim_matrix2 = 1 - cost_matrix2
    fuse_sim = sim_matrix1 * (1 + sim_matrix2) / 2
