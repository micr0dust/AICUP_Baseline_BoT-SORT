...
from tracker import my_matching as matching
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls, cam, feat=None, feat_history=50):
        self.previous_bboxes = deque(maxlen=2)
        self.cam = cam
    ...
    def xywh_to_tlbr(xywh):
        ...
        ret[:2] -= ret[2:] / 2  # Convert center x, center y to top left x, top left y
        ret[2:] += ret[:2]  # Convert width, height to bottom right x, bottom right y
        return ret
...
class BoTSORT(object):
    def __init__(self, args, frame_rate=30):
        ...
        # LSTM
        self.lstm_predict_thresh = args.LSTM_predict_thresh
    ...
    def update(self, output_results, img, camID, frameID=0, same_img=False):
        ...
        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, camID, f) for
                              (tlbr, s, c, f) in zip(dets, scores_keep, classes_keep, features_keep)]
            else:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, camID) for
        ...
        if self.args.with_reid:
            ...
            # Use LSTM predict iou with detections and ReID feature as distance
            dists = matching.fuse_two_costs(emb_dists, predict_dists)
            dists = np.where(emb_dists <= self.appearance_thresh, emb_dists, dists)
        else:
            dists = ious_dists
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, camID, f) for
